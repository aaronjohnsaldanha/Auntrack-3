...
const EditEventModal: React.FC<EditEventModalProps> = ({ event, onClose, onUpdate }) => {
  ...
  useEffect(() => {
    if (event) {
      ...
      const category = categories.find(cat => cat.name === event.category_name)
      ...
      const startTime = convertTo12Hour(format(new Date(event.start_date), 'HH:mm'))
      const endTime = convertTo12Hour(format(new Date(event.end_date), 'HH:mm'))
